...
export class HUD {
  ...
  #build() {
    ...
    this.startOverlay = q('#startOverlay');
    this.startButton = q('#startButton');
    this.pauseOverlay = q('#pauseOverlay');
    this.worldMessage = q('#worldMessage');
    this.interactionPrompt = q('#interactionPrompt');
    this.objectiveText = q('#objectiveText');
    this.subtitle = q('#subtitle');
    this.subtitleSpeaker = q('#subtitleSpeaker');
    this.subtitleText = q('#subtitleText');
    this.transition = q('#transition');
    this.transitionTitle = q('#transitionTitle');
    this.transitionSubtitle = q('#transitionSubtitle');
    this.ending = q('#ending');
    this.damageFlash = q('#damageFlash');
    this.shieldOverlay = q('#shieldOverlay');
    this.radar = q('#radar');
    this.contacts = q('#contacts');
    this.compassText = q('#compassText');
    this.contourText = q('#contourText');
    this.healthNum = q('#healthNum');
    this.sporeCount = q('#sporeCount');
    this.hpText = q('#hpText');
    this.infectionText = q('#infectionText');
    this.infectionFill = q('#infectionFill');
    this.healthCells = q('#healthCells');
    this.backpack = q('#backpack');
    this.pauseInventory = q('#pauseInventory');
    this.recipes = q('#recipes');
    this.weapons = q('#weapons');
    this.systemLine = q('#systemLine');
    this.dossierStatus = q('#dossierStatus');
    this.beltSlots = [...this.root.querySelectorAll('.weapon-slot')];
    this.belt0Name = q('#belt0Name'); this.belt1Name = q('#belt1Name');
    this.belt0Ammo = q('#belt0Ammo'); this.belt1Ammo = q('#belt1Ammo');

    for (let i = 0; i < 8; i++) {
      const cell = document.createElement('i');
      cell.className = 'hp-cell';
      this.healthCells.appendChild(cell);
    }
  ...
  bindCraft(handler) {
    this.recipes.addEventListener('click', (event) => {
      const button = event.target.closest('[data-craft]');
      if (button) handler(button.dataset.craft);
    });
  }
  bindEquip(handler) {
    this.weapons.addEventListener('click', (event) => {
      const button = event.target.closest('[data-equip]');
      if (button) handler(Number(button.dataset.slot), button.dataset.equip);
    });
  }

  setPointerLocked(locked) {
    if (locked) this.startOverlay.classList.add('hidden');
  }
  ...
  showMessage(text, seconds = 1.2) {
    this.worldMessage.textContent = text;
    this.worldMessage.classList.add('visible');
  ...
  showTransition(meta) {
    ...
    this.shell.style.setProperty('--zone', zone);
    this.shell.style.setProperty('--zoneText', zoneText);
    this.transitionTitle.textContent = `${meta.number}. ${meta.title}`;
    this.transitionSubtitle.textContent = meta.contour;
    this.transition.classList.add('visible');
    this.transitionTimer = 2.25;
    this.contourText.textContent = meta.contour.replace(' КОНТУР', '');
  }
  ...
  showEnding(kind) {
    ...
    this.ending.innerHTML = burn
      ? `<div class="ending-card burn"><b>ЗАЧИСТКА ПОДТВЕРЖДЕНА</b><p>Контур уничтожен. В официальном отчёте О-41 снова становится «аварийным биологическим агентом». Последняя запись Скальпеля заканчивается за 11 секунд до детонации.</p><small>ПРЕДУПРЕЖДЕНИЕ НЕ ВЫШЛО ЗА ПЕРИМЕТР.</small></div>`
      : `<div class="ending-card merge"><b>НОВЫЙ УЗЕЛ</b><p>Скальпель выходит из Zоны-14 один. Приборы К.О.Н.Т.У.Р. показывают норму. В следующие недели десятки людей начинают видеть один и тот же белый коридор во сне.</p><small>СИГНАЛ ПОКИНУЛ ПЕРИМЕТР.</small></div>`;
  ...
  update(dt, state) {
    ...
    if (this.messageTimer <= 0) this.worldMessage.classList.remove('visible');
    this.compassJam = Math.max(0, this.compassJam - dt);
    this.transitionTimer = Math.max(0, this.transitionTimer - dt);
    if (this.transitionTimer <= 0) this.transition.classList.remove('visible');

    const { player, infection, inventory, belt, crafting, objective, enemies, spores, levelMeta } = state;
    const health = Math.ceil(player.health);
    this.healthNum.textContent = String(health).padStart(2, '0');
    this.hpText.textContent = `${health}/${player.maxHealth}`;
    ...
    this.infectionText.textContent = `${displayedInfection}%`;
    this.infectionFill.style.width = `${infectionValue}%`;
    this.dossierStatus.textContent = infection.tier >= 3 ? 'РЕЗОНАНС: ПОДТВ.' : 'РЕЗОНАНС: ?';
    this.sporeCount.textContent = String(inventory.count('spores')).padStart(2, '0');
    this.shieldOverlay.classList.toggle('active', player.shieldTimer > 0);
    this.setObjective(objective.text);
    if (levelMeta) this.contourText.textContent = levelMeta.contour.replace(' КОНТУР', '');

    this.#updateBelt(belt, infection);
    this.#updateInventory(inventory, crafting, belt);
    this.#drawRadar(state);
  }

  #updateBelt(belt, infection) {
    const ids = belt.slots;
    const names = [this.belt0Name, this.belt1Name];
    const ammoNodes = [this.belt0Ammo, this.belt1Ammo];
    for (let i = 0; i < 2; i++) {
      const id = ids[i];
      const weapon = id ? WEAPONS[id] : null;
      names[i].textContent = weapon?.short ?? '—';
  ...
  #updateInventory(inventory, crafting, belt) {
    const slotHtml = inventory.snapshot().map((slot, index) => `
      <div class="inv-slot ${slot ? 'full' : ''}"><span>${index + 1}</span><b>${slot?.name ?? 'ПУСТО'}</b><small>${slot ? `x${slot.count}` : ''}</small></div>
    `).join('');
    this.backpack.innerHTML = slotHtml;
  ...
  #drawRadar(state) {
    ...
    ctx.strokeStyle = '#173f34'; ctx.lineWidth = 1;
    ...
    const dot = (pos, color, size = 4) => {
      const p = toRadar(pos);
      if (p.x < 2 || p.y < 2 || p.x > w - 2 || p.y > h - 2) return;
      ctx.fillStyle = color; ctx.fillRect(Math.round(p.x - size / 2), Math.round(p.y - size / 2), size, size);
    };
    ...
    ctx.fillStyle = '#d8dfd5'; ctx.fillRect(w / 2 - 2, h / 2 - 2, 4, 4);
    ctx.strokeStyle = '#d8dfd5'; ctx.beginPath(); ctx.moveTo(w / 2, h / 2); ctx.lineTo(w / 2 - Math.sin(state.player.yaw) * 12, h / 2 - Math.cos(state.player.yaw) * 12); ctx.stroke();
    ...
    this.compassText.textContent = `${String(Math.round(shownDegree)).padStart(3, '0')}°`;
    this.contacts.textContent = `КОНТ: ${state.enemies.alive().length}`;
  }
}
